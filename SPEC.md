...
- メニュー:
  - ⚠ 設定ファイルにエラー（設定ファイルにエラーがあるときだけ先頭に出し、区切り線を挟む。選ぶと
    設定ファイルを開く）。このときツールチップにも理由を出す。F5 を参照
  - ⚠ 設定に N 件の警告（項目ごとの警告があるときだけ先頭に出す。エラーの項目があればその下に並べ、
    区切り線はまとめて 1 本にする。選ぶと設定ファイルを開く）。ツールチップは変えない
  - 今すぐ休憩
...
- 読込で既定値に戻したとき（読めない・JSON として不正・ルートがオブジェクトでない）と、「休憩時間」の
  保存に失敗したときは、標準エラーに加えてトレイにも出す（F4）。表示は次に読込に成功したときに消す。
  項目ごとの警告（範囲外の丸め、`clockTimes` の除外など）は標準エラーに出し、トレイには件数だけを
  出す（F4）。件数は読込のたびに数え直す。
