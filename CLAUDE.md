# CLAUDE.md — Hitoiki（Electron 版）

Claude Code（Opus 5）向けの作業指示。仕様は SPEC.md が正。矛盾を見つけたら SPEC.md を優先し、
矛盾の内容を報告する。
...
npm test                       # vitest run
npm start                      # electron dist/main.js。作者が手元で実行する。Claude は実行しない
npm run dist                   # electron-builder。依頼があるときだけ
```

package.json の `scripts` は上記 5 つに限る。
...
- 標準ツール（Read / Edit / Write / Bash / Grep）だけを使う。
- **プラグイン、スキル、サブエージェント、MCP サーバーは使わない。**
...
- 例外を握るのは Config の読み書きだけ（読込は既定値にフォールバック、書込は何もしない。どちらも標準エラーに理由を出す）。
  ...
  まだ使っているウィンドウのエラーは投げ直す（`overlay.ts` の `addDisplay`）。
- 識別子とコード内コメントは英語。ユーザーとのやり取りは日本語。

## 手動検証（作者が行う）

Claude は GUI を起動できないので、以下は作者が確認する。報告には「作者が確認する項目」として書く。
